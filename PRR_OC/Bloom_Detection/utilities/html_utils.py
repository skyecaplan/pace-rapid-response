...
Authors:
    ...
    Matthew Kehrli, NASA/GSFC, 2026-02-18
...
def write_full_html(day, ofilepath):
    ...
    h1 = 'PACE OCI Potential Bloom Dashboard: '+day
    ...
    with open(ofilepath, "w") as f:
            ...
            f.write(f"""<p>The PACE OCI Potential Bloom Dashboard identifies L2 PACE OCI granules that exhibit substantial
                    chlorophyll-a anomalies. The anomaly values are calculated from L3 daily 
                    composites relative to the preceding 15-day L3 composite mean:</p>\n
                    
                    <div style="text-align: center; margin: 20px 0;">
                    <p style="font-size: 16px;">
                    <strong>Chlorophyll-<em>a</em> anomaly</strong> = 
                    <span style="display: inline-block;">
                    <span style="display: block; text-align: center; border-bottom: 1px solid #000; padding: 0 5px;">
                                    (daily chlorophyll-<em>a</em> − 15-day running chlorophyll-<em>a</em> mean)
                    </span>
                    <span style="display: block; text-align: center; padding: 0 5px;">
                                    15-day running chlorophyll-<em>a</em> mean
                    </span>
                    </span>
                            × 100%
                    </p>
                    ...
                    <p>Potential bloom-forming regions are detected in the L3 data by 
                    partitioning the scene into 100x100 L3-pixel blocks and flagging blocks where at least 10% of 
                    pixels show a chlorophyll-a anomaly greater than 100%. Flagged blocks serve as bounding boxes 
                    to locate corresponding L2 granules for the same day using NASA's earthaccess data search tool. These L2 granules are shown with optical and 
                    biogeochemical parameters overlaid on true-color imagery with embedded red squares to highlight the 
                    L3 blocks that contain the substantial chlorophyll-a anomalies.</p>\n""")

            f.write(f"<div>Developed by Graham Trolley and Matthew Kehrli, Ocean Ecology Laboratory, NASA Goddard Space Flight Center</div>\n")
            ...
            capts = ['Identified Granules on ' + day + ': 15-day Relative Anomaly [%]']
            sizes = ['large']
            write_html_image_row(f, imgs, capts, sizes, fill_width=True, equal_height=False)# single-image row
            ...
            for granule in granule_folders:
                ...
                a_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_a_overlay.png', factor = granule_images_shrink_factor)
                anw_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_anw_overlay.png', factor = granule_images_shrink_factor)
                aph_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_aph_overlay.png', factor = granule_images_shrink_factor)
                adg_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_adg_overlay.png', factor = granule_images_shrink_factor)
                bb_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_bb_overlay.png', factor = granule_images_shrink_factor)
                bbp_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_bbp_overlay.png', factor = granule_images_shrink_factor)
                Kd_im = fig_to_base64('figures/'+day+'/png/'+granule+'/'+granule+'_Kd_overlay.png', factor = granule_images_shrink_factor)
                ...
                imgs = [outline_im, anom_img, chlor_a_im, nflh_im]
                capts = ['&nbsp;', 'Chl-a anomaly', 'Chlorophyll-a', 'NFLH']
                ...
                # Third row - also hidden by default, shows more parameters.
                f.write(f"<div style='height: 20px;'></div>\n") # add some spacing between rows
                imgs = [a_im, bb_im, anw_im, bbp_im]
                capts = [' Absorption Coefficient at 442 nm', 'Backscattering Coefficient at 442 nm', 'Non-water Absorption Coefficient at 442 nm', 'Particulate Backscattering Coefficient at 442 nm']
                sizes = ['large', 'large', 'large', 'large']
                write_html_image_row(f, imgs, capts, sizes)

                # Fourth row - also hidden by default, shows more parameters.
                f.write(f"<div style='height: 20px;'></div>\n") # add some spacing between rows
                imgs = [aph_im, adg_im, Kd_im]
                capts = ['Phytoplankton Absorption Coefficient at 442 nm', 'Non-Phytoplankton Absorption Coefficient at 442 nm', 'Diffuse Attenuation Coefficient for Downwelling Irradiance at 490 nm']
